...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ 
    ...
    setShifts(prev => prev.map(s => {
      if (s.id === shiftId) {
        return {
          ...
          status: 'pending' // Move back to pending so supervisor can start it
        };
      }
      return s;
    }));
  ...
  const handleForceStop = (shiftId: string, cleanerName: string) => {
    if (!window.confirm(`FORCE TERMINATE SESSION for ${cleanerName.toUpperCase()}?\n\nUse this only if the staff member is stuck or left without finishing. They will be able to start new shifts immediately.`)) return;
