...
const CleanerPortal: React.FC<CleanerPortalProps> = ({ 
  ...
  const handleIncidentSubmit = () => {
    ...
    // Relaxation: Missing items (especially laundry) might not have photo evidence of the "absence"
            ...
            <div className="bg-white w-full max-w-xl rounded-[2.5rem] md:rounded-[48px] shadow-2xl relative overflow-hidden flex flex-col max-h-[90vh]">
              ...
              <div className="p-6 md:p-10 pt-4 md:pt-6 border-t border-gray-50 flex gap-3 md:gap-4 bg-white">
                 ...
                 <button 
                  ...
                  className="flex-[2] py-4 md:py-6 bg-indigo-600 text-white font-black rounded-2xl md:rounded-[24px] text-[9px] md:text-[11px] uppercase tracking-[0.2em] md:tracking-[0.3em] shadow-2xl transition-all active:scale-95 hover:bg-indigo-700 disabled:opacity-50"
                 >
                  request
                 </button>
              </div>
        ...
        <div className="bg-white border border-teal-100 p-5 md:p-8 rounded-[2.5rem] md:rounded-[40px] space-y-6 md:space-y-8 shadow-xl">
           ...
           <div className="pt-2 md:pt-4 space-y-4">
              {!isLocationVerified ? (
                ...
                <button onClick={handleStartShift} className="w-full py-4 md:py-5 rounded-2xl md:rounded-3xl bg-teal-600 text-white font-black uppercase text-[10px] md:text-[11px] tracking-[0.3em] md:tracking-[0.4em] shadow-2xl active:scale-95 transition-all hover:bg-teal-700">
  ...
  if (currentStep === 'active' && activeShift) {
    ...
    return (
      <div className="pb-32 animate-in fade-in duration-500 max-w-3xl mx-auto text-left">
         <div className="bg-[#0D9488] p-6 md:p-10 mb-6 md:mb-8 rounded-b-[2.5rem] md:rounded-b-[60px] text-white shadow-2xl space-y-3">
            <div className="space-y-0.5">
               ...
               <div className="flex items-center gap-2">
                  ...
                  <p className="text-[8px] md:text-[9px] font-black text-white/60 uppercase tracking-[0.3em]">{isAudit ? 'Audit Session' : 'Live Mission'} • {Math.floor(timer / 3600)}h {Math.floor((timer % 3600) / 60)}m</p>
               </div>
            </div>
         </div>
         ...
         <div className="px-3 md:px-4 space-y-8 md:space-y-12">
            <div className="grid grid-cols-2 gap-2.5 md:gap-3">
               ...
               <button onClick={() => setReportModalType('maintenance')} className="h-20 md:h-24 bg-slate-50 border-2 border-slate-300 text-slate-700 flex flex-col items-center justify-center gap-1.5 md:gap-2 rounded-2xl md:rounded-[28px] transition-all active:scale-95 hover:bg-slate-100 shadow-sm">
                  ...
                  <span className="text-[8px] font-black uppercase tracking-widest">Maintenance</span>
               </button>
               <button onClick={() => setReportModalType('damage')} className="h-20 md:h-24 bg-amber-50 border-2 border-amber-200 text-amber-700 flex flex-col items-center justify-center gap-1.5 md:gap-2 rounded-2xl md:rounded-[28px] transition-all active:scale-95 hover:bg-amber-100 shadow-sm">
                  ...
                  <span className="text-[8px] font-black uppercase tracking-widest">Damages</span>
               </button>
                     ...
                     <div key={task.id} className={`border rounded-3xl md:rounded-[32px] p-5 md:p-6 transition-all shadow-sm ${hasPhoto ? 'bg-teal-50 border-teal-500' : 'bg-white border-slate-200'}`}>
                        <div className="flex justify-between items-start mb-4 text-left">
                           <div className="space-y-1 flex-1">
                              ...
                              {task.isMandatory && !hasPhoto && attemptedNext && <span className="text-[7px] md:text-[8px] font-black text-red-600 uppercase tracking-widest">Evidence Required</span>}
                              {!hasPhoto && <p className="text-[7px] md:text-[8px] font-black text-slate-400 uppercase tracking-[0.3em]">Evidence Pending</p>}
                              {hasPhoto && <p className="text-[7px] md:text-[8px] font-black text-teal-600 uppercase tracking-[0.3em]">Satisfied</p>}
                           </div>
                           <div className={`shrink-0 ml-3 ${hasPhoto ? 'text-teal-600' : 'text-slate-300'}`}>
                             {hasPhoto ? (
                               <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="4"><polyline points="20 6 9 17 4 12"/></svg>
                             ) : (
                               <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="4"><circle cx="12" cy="12" r="10"/></svg>
                             )}
                           </div>
                        </div>
                ...
                <div className="grid grid-cols-1 gap-4 md:gap-5">
                  {[
                    ...
                    <div key={idx} className="flex items-center gap-3 md:gap-4">
                      ...
                      <p className="text-[9px] md:text-[11px] font-black uppercase tracking-widest leading-none">{item}</p>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="fixed inset-0 bg-black/90 z-[1000] flex items-end sm:items-center justify-center p-2 sm:p-4 backdrop-blur-sm animate-in slide-in-from-bottom-5">
               <div className="bg-white w-full max-w-md rounded-[2.5rem] md:rounded-[50px] space-y-6 md:space-y-8 shadow-2xl relative text-center overflow-hidden">
                  ...
                  <div className="p-8 md:p-10 pb-4 md:pb-4 space-y-1.5 md:space-y-2">
                     <h3 className="text-2xl md:text-3xl font-brand font-extrabold uppercase tracking-tighter text-slate-900 leading-none">{showMessReport ? 'Extra Hours Report' : reportModalType?.toUpperCase()}</h3>
                    ...
                    <div className="px-8 md:px-10 pb-2 md:pb-4">
                       ...
                       <div className="flex p-1 bg-slate-50 border border-slate-200 rounded-xl md:rounded-2xl shadow-inner">
                          <button 
                            ...
                            className={`flex-1 py-2.5 md:py-3 rounded-lg md:rounded-xl text-[8px] md:text-[9px] font-black uppercase tracking-widest transition-all ${missingCategory === 'apartment' ? 'bg-white text-indigo-600 shadow-sm border border-slate-100' : 'text-slate-400 hover:text-slate-600'}`}
                          >
                             Apartment
                          </button>
                          <button 
                            onClick={() => setMissingCategory('laundry')}
                            className={`flex-1 py-2.5 md:py-3 rounded-lg md:rounded-xl text-[8px] md:text-[9px] font-black uppercase tracking-widest transition-all ${missingCategory === 'laundry' ? 'bg-white text-indigo-600 shadow-sm border border-slate-100' : 'text-slate-400 hover:text-slate-600'}`}
                          >
                             Laundry
                          </button>
                       </div>
