  ...
  const worksheetData = useMemo(() => {
    ...
    const rows = filtered.map(s => {
      ...
      if (s.approvalStatus === 'approved') {
        if (user.role === 'cleaner' && user.paymentType === 'Per Clean' && prop) {
          const teamCount = s.userIds.length || 1;
          const pieceRate = (prop.serviceRates?.[s.serviceType] || prop.cleanerPrice) / teamCount;
          payout = Math.max(basePay, pieceRate);
        } else if (s.serviceType === 'TO CHECK APARTMENT' && user.role === 'supervisor' && prop) {
          payout = prop.cleanerAuditPrice || 0;
        } else if (s.serviceType === 'TO FIX' && s.fixWorkPayment) {
          payout = s.fixWorkPayment;
        } else {
          payout = basePay;
        }
      } else {
        // Rejected or Pending shifts usually revert to base hourly for safety, or 0 if pending
        payout = s.approvalStatus === 'rejected' ? basePay : 0;
      }
    ...
    <div className="space-y-8 animate-in fade-in duration-700 text-left max-w-6xl mx-auto">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 px-1">
        ...
        <select 
          ...
          className="bg-white border border-slate-200 rounded-xl px-6 py-3 text-[10px] font-black uppercase tracking-widest outline-none focus:border-teal-500 shadow-sm"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
         <div className="bg-[#1E293B] p-8 rounded-[2.5rem] text-white shadow-xl flex justify-between items-center relative overflow-hidden group">
            <div className="space-y-1 relative z-10">
               <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Period Activity</p>
               <p className="text-4xl font-bold font-brand tracking-tighter">{worksheetData.totalHours} <span className="text-sm text-slate-500">HRS</span></p>
            </div>
            ...
            <div className="absolute -right-4 -bottom-4 opacity-5 pointer-events-none group-hover:rotate-12 transition-transform duration-1000">
               <svg width="150" height="150" viewBox="0 0 24 24" fill="white"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
            </div>
         </div>
         ...
         <div className="bg-white p-8 rounded-[2.5rem] border border-teal-100 shadow-xl flex justify-between items-center relative overflow-hidden group">
            <div className="space-y-1 relative z-10">
               <p className="text-[8px] font-black text-teal-600 uppercase tracking-widest">Est. Gross Earnings</p>
               <p className="text-4xl font-bold font-brand tracking-tighter text-[#0D9488]">€{worksheetData.totalEarnings}</p>
            </div>
            ...
            <div className="absolute -right-4 -bottom-4 opacity-10 pointer-events-none group-hover:rotate-12 transition-transform duration-1000 text-teal-600">
               <svg width="150" height="150" viewBox="0 0 24 24" fill="currentColor"><rect x="2" y="6" width="20" height="12" rx="2"/><circle cx="12" cy="12" r="2"/><path d="M6 12h.01M18 12h.01"/></svg>
            </div>
         </div>
      </div>
          ...
          <table className="w-full text-left">
            <thead className="bg-slate-50 border-b border-slate-100">
              <tr>
                ...
                <th className="px-8 py-5 text-[9px] font-black text-slate-400 uppercase tracking-widest">Shift Timeline</th>
                <th className="px-8 py-5 text-[9px] font-black text-slate-400 uppercase tracking-widest text-center">Status</th>
                <th className="px-8 py-5 text-[9px] font-black text-slate-400 uppercase tracking-widest text-right">Payout (€)</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-50">
              {worksheetData.rows.length === 0 ? (
                <tr>
                  <td colSpan={4} className="px-8 py-20 text-center text-slate-300 italic text-[10px] font-black uppercase tracking-widest">No completed sessions found for this period.</td>
                </tr>
                ...
                <tr key={row.id} className="hover:bg-teal-50/20 transition-colors">
                  <td className="px-8 py-6">
                    ...
                    <div className="flex items-center gap-2 mt-1.5">
                       <span className="text-[8px] font-black text-slate-300 uppercase tracking-widest">{row.date}</span>
                       <span className="w-1 h-1 rounded-full bg-slate-200"></span>
                       <span className="text-[8px] font-black text-teal-600 uppercase tracking-widest">{row.service}</span>
                    </div>
                  </td>
                  <td className="px-8 py-6">
                    <p className="text-[10px] font-bold text-slate-700">{row.time}</p>
                    <p className="text-[8px] font-black text-slate-300 uppercase tracking-widest mt-0.5">{row.hours} Hours Logged</p>
                  </td>
                  ...
                  <td className="px-8 py-6 text-right">
                    <p className="text-sm font-black text-slate-900 tracking-tight">€{row.payout}</p>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <p className="text-[8px] text-slate-300 font-bold uppercase tracking-[0.5em] text-center px-4 leading-relaxed">
         Estimations shown are gross figures. Official payslips with tax deductions are available in the Settings tab.
      </p>
    </div>
  );
};
